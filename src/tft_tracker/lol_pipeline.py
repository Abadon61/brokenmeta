...
PING_KEYS = ["allInPings", "assistMePings", "basicPings", "commandPings", "dangerPings", "enemyMissingPings", "enemyVisionPings",
             "getBackPings", "holdPings", "needVisionPings", "onMyWayPings", "pushPings", "retreatPings", "visionClearedPings"]
MIN_ROWS_PER_RANK_ROLE = 150   # below this a (tier, role) average is too noisy to publish
MIN_REAL_GAME_SECONDS = 300    # remakes / early surrenders would skew every per-minute average


def _participant_metrics(p: dict, team_kills: int, duration_min: float) -> dict[str, float]:
    """One participant's per-game / per-minute numbers. Definitions mirror
    lol-worker/src/index.ts::extractMatch/buildQueueBlock exactly, so a
    looked-up player is compared with the same maths."""
    ch = p.get("challenges") or {}
    cs = (p.get("totalMinionsKilled") or 0) + (p.get("neutralMinionsKilled") or 0)
    return {
        "csPerMin": cs / duration_min,
        "goldPerMin": (p.get("goldEarned") or 0) / duration_min,
        "dpm": (p.get("totalDamageDealtToChampions") or 0) / duration_min,
        "killParticipation": ((p.get("kills") or 0) + (p.get("assists") or 0)) / max(1, team_kills) * 100,
        "visionPerMin": (p.get("visionScore") or 0) / duration_min,
        "wardsPlaced": p.get("wardsPlaced") or 0,
        "wardsKilled": p.get("wardsKilled") or 0,
        "controlWards": p.get("visionWardsBoughtInGame") or 0,
        "turrets": p.get("turretTakedowns") or 0,
        "plates": ch.get("turretPlatesTaken") or 0,
        "dragons": ch.get("dragonTakedowns") or 0,
        "barons": ch.get("baronTakedowns") or 0,
        "heralds": ch.get("riftHeraldTakedowns") or 0,
        "objDmgPerMin": (p.get("damageDealtToObjectives") or 0) / duration_min,
        "damageTakenPerMin": (p.get("totalDamageTaken") or 0) / duration_min,
        "mitigatedPerMin": (p.get("damageSelfMitigated") or 0) / duration_min,
        "ccTime": p.get("timeCCingOthers") or 0,
        "deadPct": (p.get("totalTimeSpentDead") or 0) / (duration_min * 60) * 100,
        "healPerMin": (p.get("totalHeal") or 0) / duration_min,
        "pingsPerGame": sum(p.get(k) or 0 for k in PING_KEYS),
    }


def build_rank_averages(matches_by_tier: dict[str, list[dict]]) -> dict:
    """tier -> role -> mean of every metric above, over every participant of
    every sampled match of that tier (each participant weighs the same).
    "all" pools the five roles as a fallback. Replaces the hand-written
    approximate table the worker used to ship -- these are measured numbers.
    Matches sampled at tier T also contain their lobby's other players, so a
    tier's figure is "players in games sampled at that tier", same caveat as
    the tier list itself."""
    out: dict[str, dict] = {}
    for tier, matches in matches_by_tier.items():
        rows_by_role: dict[str, list[dict[str, float]]] = defaultdict(list)
        seen: set[str] = set()
        for m in matches:
            mid = m.get("metadata", {}).get("matchId")
            info = m.get("info", {})
            if not mid or mid in seen or (info.get("gameDuration") or 0) < MIN_REAL_GAME_SECONDS:
                continue
            seen.add(mid)
            ...
            for p in participants:
                ...
                if not role:
                    continue
                rows_by_role[role].append(_participant_metrics(p, team_kills[p.get("teamId")], duration_min))

        def mean(rows: list[dict[str, float]]) -> dict:
            keys = rows[0].keys()
            return {"n": len(rows), "metrics": {k: round(sum(r[k] for r in rows) / len(rows), 2) for k in keys}}

        all_rows = [r for rows in rows_by_role.values() for r in rows]
        if not all_rows:
            continue
        roles = {role: mean(rows) for role, rows in rows_by_role.items() if len(rows) >= MIN_ROWS_PER_RANK_ROLE}
        out[tier] = {"all": mean(all_rows), "roles": roles}
    return out


def write_worker_rank_averages(averages: dict, path: Path) -> None:
    """Bakes the averages into the worker as a typed module (no runtime
    fetch = no extra subrequest, which the worker is short on). A live
    collection run followed by `wrangler deploy` is what refreshes it."""
    body = {tier: {"all": v["all"], **{role: r for role, r in v["roles"].items()}} for tier, v in averages.items()}
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "// AUTO-GENERATED by src/tft_tracker/lol_pipeline.py (--worker-rank-averages-out). Do not edit by hand.\n"
        "// Measured means per rank tier and role over the participants of sampled ranked matches.\n"
        f"export const RANK_AVERAGES_GENERATED_AT = {json.dumps(datetime.now(timezone.utc).date().isoformat())};\n"
        "export interface RankAverage { n: number; metrics: Record<string, number>; }\n"
        f"export const RANK_AVERAGES: Record<string, Record<string, RankAverage>> = {json.dumps(body, separators=(',', ':'))};\n",
        encoding="utf-8",
    )
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--rank-averages-out", default=f"{config.OUTPUT_DIR}/lol_rank_averages.json")
    p.add_argument("--worker-rank-averages-out", default=str(Path(__file__).resolve().parents[2] / "lol-worker" / "src" / "rankAveragesData.ts"),
                   help="Also bake the rank averages into the worker as a TypeScript module (needs a wrangler deploy to go live).")
...
def main(argv=None) -> None:
    ...
    matches_by_tier: dict[str, list[dict]] = defaultdict(list)
    if args.from_cache:
    ...
    if matches_by_tier:
        rank_averages = build_rank_averages(matches_by_tier)
        ra_path = Path(args.rank_averages_out)
        ra_path.parent.mkdir(parents=True, exist_ok=True)
        ra_path.write_text(json.dumps({"generated_at": datetime.now(timezone.utc).isoformat(), "tiers": rank_averages}, indent=2), encoding="utf-8")
        write_worker_rank_averages(rank_averages, Path(args.worker_rank_averages_out))
        for tier, v in rank_averages.items():
            print(f"  rank averages {tier}: {v['all']['n']} participant rows, {len(v['roles'])} roles with enough data.")
        print(f"Rank averages written to {ra_path} and {args.worker_rank_averages_out}.")
