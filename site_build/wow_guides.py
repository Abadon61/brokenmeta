...
TXT = {
    "fr": {
        ...
        "sim_method": [
            ...
            "Le calculateur par formule ci-dessus reste volontairement plus simple : une seule arme, aucune Rage, Bloodthirst et Whirlwind toujours disponibles. C'est un plafond théorique sans temps mort ni double maniement. La simulation applique le vrai goulot d'étranglement de la Rage et le double maniement réel, donc son DPS peut finir au-dessus ou en dessous du total du calculateur selon vos statistiques : ce n'est pas une incohérence entre les deux méthodes.",
        ...
        "sim_src_dws": "Rangs de Spécialisation Ambidextrie (WoW: Forever) : ",
    },
    "en": {
        ...
        "sim_method": [
            ...
            "Dual-wielding is now modeled with two independent weapons (main hand and off hand), each with its own swing timer and its own Rage generation. Real classic-WoW mechanics: the off-hand weapon deals 50% of its usual damage, and dual-wielding adds a 19% miss-chance penalty to both weapons' normal swings (on top of your usual hit chance) — this penalty doesn't apply to Bloodthirst, Whirlwind or Heroic Strike, which are special attacks. When dual-wielding, Whirlwind strikes with both weapons (a separate roll for each), matching real gameplay.",
            ...
            "The formula-based calculator above stays deliberately simpler: a single weapon, no Rage, Bloodthirst and Whirlwind always available. It's a theoretical ceiling with no downtime and no dual-wielding. The simulation applies the real Rage bottleneck and real dual-wielding, so its DPS can land above or below the calculator's total depending on your stats: that's not a disagreement between the two methods.",
        ...
        "sim_src_dws": "Dual Wield Specialization ranks (WoW: Forever): ",
    },
}
