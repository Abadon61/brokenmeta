...
TXT = {
    "fr": {
        ...
        "sim_method": [
            ...
            "Le calculateur par formule ci-dessus reste volontairement plus simple : une seule arme, aucune Rage, aucun proc, Bloodthirst et Whirlwind toujours disponibles. C'est un plafond théorique sans temps mort. La simulation applique le vrai goulot d'étranglement de la Rage, le double maniement réel et ces procs, donc son DPS peut finir au-dessus ou en dessous du total du calculateur selon vos statistiques : ce n'est pas une incohérence entre les deux méthodes.",
            "Autres simplifications non modélisées : la Rage gagnée en subissant des dégâts (non pertinente pour un DPS qui ne tank pas), le remboursement partiel de Rage d'Heroic Strike en cas d'échec (le coût plein est prélevé à chaque tentative), et Deep Wounds (un vrai saignement du talent, mais de l'arbre Armes, pas Fureur : hors sujet pour cet outil tant qu'aucune source ne confirme un hybride Fureur/Armes pour Forever au niveau 60).",
        ...
        "sim_src_dws": "Rangs de Spécialisation Ambidextrie, Rafale et Colère déchaînée (WoW: Forever) : ",
    ...
    "en": {
        ...
        "sim_method": [
            ...
            "Dual-wielding is modeled with two independent weapons (main hand and off hand), each with its own swing timer and its own Rage generation. Real classic-WoW mechanics: the off-hand weapon deals 50% of its usual damage, and dual-wielding adds a 19% miss-chance penalty to both weapons' normal swings (on top of your usual hit chance) — this penalty doesn't apply to Bloodthirst, Whirlwind or Heroic Strike, which are special attacks. When dual-wielding, Whirlwind strikes with both weapons (a separate roll for each), matching real gameplay.",
            ...
            "Two real Fury-tree procs from WoW: Forever are now modeled. Flurry (5 ranks): +5% attack speed per rank for your next 3 swings after a melee critical strike (auto-attack or a special ability like Bloodthirst/Whirlwind), with charges expiring after 15 unused seconds and resetting to a fresh 3 on any new crit; the speed bonus only ever applies to normal weapon swings, never to special abilities, which stay on their own fixed cooldowns. Unbridled Wrath (5 ranks): +12% chance per rank to generate 1 extra Rage whenever you deal melee damage with a weapon. Both come from WoW: Forever's real Fury-tree data, with their exact ranks and percentages.",
            ...
            "The formula-based calculator above stays deliberately simpler: a single weapon, no Rage, no procs, Bloodthirst and Whirlwind always available. It's a theoretical ceiling with no downtime. The simulation applies the real Rage bottleneck, real dual-wielding and these procs, so its DPS can land above or below the calculator's total depending on your stats: that's not a disagreement between the two methods.",
            "Other simplifications not modeled: Rage gained from taking damage (not relevant for a DPS that isn't tanking), Heroic Strike's partial Rage refund on a miss (the full cost is charged on every attempt), and Deep Wounds (a real bleed talent, but from the Arms tree, not Fury: out of scope for this tool until a source confirms a Fury/Arms hybrid build for Forever at level 60).",
        ...
        "sim_src_dws": "Dual Wield Specialization, Flurry and Unbridled Wrath ranks (WoW: Forever): ",
