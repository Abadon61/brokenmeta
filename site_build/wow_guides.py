...
_SHARE = {
    "fr": {
        ...
        "sh_steps": [
            "En jeu, activez le partage : <code>/bmw share on</code> (ou onglet Données du hub), puis déconnectez-vous ou tapez <code>/reload</code> pour que le jeu écrive le fichier.",
            "Retrouvez le fichier <code>BrokenMetaWeights.lua</code> dans <code>World of Warcraft\\_classic_beta_\\WTF\\Account\\&lt;votre compte&gt;\\SavedVariables\\</code>.",
            "Déposez-le ci-dessous, vérifiez l'aperçu, cochez la case de consentement et envoyez.",
        ],
    ...
    "en": {
        ...
        "sh_steps": [
            "In game, turn sharing on: <code>/bmw share on</code> (or the hub's Data tab), then log out or type <code>/reload</code> so the game writes the file.",
            "Find <code>BrokenMetaWeights.lua</code> in <code>World of Warcraft\\_classic_beta_\\WTF\\Account\\&lt;your account&gt;\\SavedVariables\\</code>.",
            "Drop it below, check the preview, tick the consent box and send.",
        ],
