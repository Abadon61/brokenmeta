...
TXT = {
    "fr": {
        ...
        "sim_method": [
            "Cette simulation modélise Bloodthirst, Whirlwind et Heroic Strike avec une vraie réserve de Rage : Bloodthirst coûte 30 Rage, Whirlwind 25 Rage, Heroic Strike 15 Rage (coûts réels lus sur les infobulles Wowhead de WoW: Forever, sorts de rang maximum). Bloodthirst reste toujours prioritaire dès qu'il est disponible ; Heroic Strike se déclenche dès que la Rage le permet (son propre coût de 15), pas seulement sur un excédent. Ce n'était pas le réglage d'origine : un seuil de 50 Rage a d'abord été choisi sans être testé, en supposant que Heroic Strike ne devait consommer que de la Rage « en trop ». En testant directement chaque seuil de 15 à 50 dans ce simulateur, le DPS baisse continûment à mesure que le seuil augmente (environ 190 DPS à 15, contre 127 à 50, aux statistiques par défaut) : Heroic Strike rend plus de dégâts par point de Rage que Whirlwind en mono-cible, donc dépenser la Rage dessus dès que possible bat le fait de la garder pour Whirlwind. Ce n'est toujours qu'un choix de priorité de rotation, pas une règle du jeu — mais un choix maintenant vérifié par la simulation elle-même plutôt que supposé.",
    ...
    "en": {
        ...
        "sim_method": [
            "This simulation models Bloodthirst, Whirlwind and Heroic Strike on a real Rage pool: Bloodthirst costs 30 Rage, Whirlwind 25 Rage, Heroic Strike 15 Rage (real costs read from WoW: Forever's own Wowhead tooltips, max-rank spells). Bloodthirst always stays top priority whenever available; Heroic Strike fires as soon as Rage allows it at all (its own 15-Rage cost), not just on a surplus. That wasn't the original setting: a 50-Rage threshold was first chosen without testing it, on the assumption that Heroic Strike should only spend \"leftover\" Rage. Testing every threshold from 15 to 50 directly in this simulator showed DPS falling off continuously as the threshold rises (roughly 190 DPS at 15 versus 127 at 50, at default stats): Heroic Strike returns more damage per Rage point than Whirlwind in a single-target fight, so spending Rage on it as early as possible beats saving it for Whirlwind. Still just a rotation priority choice, not a game rule — but one now verified by the simulation itself rather than assumed.",
