...
TXT = {
    "fr": {
        ...
        "sim_method": [
            ...
            "Important : cette simulation ne modélise qu'une seule arme (un seul timer de coup), alors qu'un Guerrier Fureur manie deux armes en jeu réel. Chaque arme génère de la Rage indépendamment en combat réel, donc ce modèle sous-estime la vraie génération de Rage d'un double maniement : Bloodthirst et Whirlwind peuvent partir moins souvent ici que dans un vrai combat (Heroic Strike, lui, ne se déclenche quasiment jamais aux statistiques par défaut). Le DPS simulé est donc une estimation plutôt basse, pas un chiffre validé — modéliser les deux armes séparément est la prochaine étape logique.",
            "Le calculateur par formule ci-dessus ignore complètement la Rage : il suppose Bloodthirst et Whirlwind toujours disponibles, un plafond théorique sans aucun temps mort. La simulation applique le vrai goulot d'étranglement de la Rage, donc le DPS simulé finit souvent inférieur au total du calculateur, pas supérieur : ce n'est pas une incohérence entre les deux méthodes, c'est la limite de Rage qui se manifeste enfin.",
            "Autres simplifications non modélisées : la Rage gagnée en subissant des dégâts (non pertinente pour un DPS qui ne tank pas), et le remboursement partiel de Rage d'Heroic Strike en cas d'échec (le coût plein est prélevé à chaque tentative).",
    ...
    "en": {
        ...
        "sim_method": [
            ...
            "Important: this simulation only models a single weapon (one swing timer), while a real Fury Warrior dual-wields two. Each weapon generates Rage independently in real combat, so this model understates the real Rage income of dual-wielding: Bloodthirst and Whirlwind can fire less often here than in a real fight (Heroic Strike barely fires at all at default stats). The simulated DPS is therefore a fairly low estimate, not a validated number — modeling both weapons separately is the logical next step.",
            "The formula-based calculator above ignores Rage entirely: it assumes Bloodthirst and Whirlwind are always available, a theoretical ceiling with zero downtime. The simulation applies the real Rage bottleneck instead, so the simulated DPS often ends up lower than the calculator's total, not higher: that's not a disagreement between the two methods, it's the Rage constraint finally showing up.",
            "Other simplifications not modeled: Rage gained from taking damage (not relevant for a DPS that isn't tanking), and Heroic Strike's partial Rage refund on a miss (the full cost is charged on every attempt).",
