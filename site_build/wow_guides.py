...
TXT = {
    "fr": {
        ...
        "sim_method": [
            ...
            "Le calculateur par formule ci-dessus reste volontairement plus simple : une seule arme, aucune Rage, aucun proc, aucun talent, Bloodthirst et Whirlwind toujours disponibles. C'est un plafond théorique sans temps mort. La simulation applique le vrai goulot d'étranglement de la Rage, le double maniement réel, ces procs et Mort désirée, donc son DPS peut finir au-dessus ou en dessous du total du calculateur selon vos statistiques : ce n'est pas une incohérence entre les deux méthodes.",
            "Cet outil ne couvre pas tous les sorts du Guerrier : seulement ceux d'une rotation Fureur soutenue mono-cible. Volontairement laissés de côté : Execute (utilisable seulement sous 20% de vie de la cible, aucune phase d'exécution modélisée), Overpower (nécessite que la cible vienne d'esquiver, aucune défense de cible modélisée), Rend et Cleave (outils Armes/zone, hors rotation Fureur mono-cible), les capacités purement utilitaires ou défensives (immunité à la Peur, génération de Rage hors combat, etc., sans contribution DPS soutenue), Enrage (un vrai talent Fureur qui se déclenche en subissant des dégâts, mais cet outil ne modélise aucun dégât reçu), et Deep Wounds (un vrai saignement, mais de l'arbre Armes, pas Fureur : hors sujet tant qu'aucune source ne confirme un hybride Fureur/Armes pour Forever au niveau 60).",
        ...
        "sim_src_dws": "Rangs de Spécialisation Ambidextrie, Rafale, Colère déchaînée et Coups déchaînés (WoW: Forever) : ",
    ...
    "en": {
        ...
        "sim_method": [
            ...
            "The formula-based calculator above stays deliberately simpler: a single weapon, no Rage, no procs, no talents, Bloodthirst and Whirlwind always available. It's a theoretical ceiling with no downtime. The simulation applies the real Rage bottleneck, real dual-wielding, these procs and Death Wish, so its DPS can land above or below the calculator's total depending on your stats: that's not a disagreement between the two methods.",
            "This tool doesn't cover every Warrior spell: only what a sustained single-target Fury rotation actually uses. Deliberately left out: Execute (only usable below 20% target health, no execute phase modeled), Overpower (needs the target to have just dodged, no target defenses modeled), Rend and Cleave (Arms/AoE tools, outside a single-target Fury rotation), purely utility or defensive abilities (Fear immunity, out-of-combat Rage generation, etc., with no sustained DPS contribution), Enrage (a real Fury talent that procs off taking damage, but this tool has no incoming-damage model), and Deep Wounds (a real bleed, but from the Arms tree, not Fury: out of scope until a source confirms a Fury/Arms hybrid build for Forever at level 60).",
        ...
        "sim_src_dws": "Dual Wield Specialization, Flurry, Unbridled Wrath and Raging Blows ranks (WoW: Forever): ",
